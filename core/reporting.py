...
def allocation_table(tickers, weights, universe) -> pd.DataFrame:
    df = pd.DataFrame(
        {
            "Ticker": tickers,
...
def benchmark_probability_table(final_values, initial_investment) -> pd.DataFrame:
    rows = []
    for name, ann_ret in BENCHMARK_ASSUMPTIONS.items():
        final_benchmark = initial_investment * (1 + ann_ret)
        prob = float((final_values > final_benchmark).mean() * 100)
        rows.append(
            {
                "Benchmark": name,
...
def percentile_table(final_values, initial_investment) -> pd.DataFrame:
    p = [1, 5, 10, 25, 50, 75, 90, 95, 99]
    values = np.percentile(final_values, p)
    returns = (values / initial_investment - 1) * 100
    return pd.DataFrame({"Percentile": p, "Terminal Value ($)": values, "Return (%)": returns})
